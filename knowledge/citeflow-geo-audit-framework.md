...
```
技术地基：
  Discoverability  — AI 能找到你吗？
  Structured Data — 数据格式对 AI 友好吗？
...
- `robots.txt` 是否误封关键页面？
- `robots.txt` 是否放行了所有主要 AI 爬虫？（GPTBot, ClaudeBot, PerplexityBot, Google-Extended, CCBot, meta-externalagent, cohere-ai, Bytespider）
- SPA 页面是否返回空壳（JS 渲染问题）？
- 多语言版本是否各自可索引？
- 网站是否有 `llms.txt`？（AI 爬虫导航文件）
...
- 确认 robots.txt 放行全部主要 AI 爬虫（完整列表见 `templates/technical-schema-robots.md`）
- SPA → SSR/SSG 改造，或至少确保关键页面预渲染
- 多语言站点：确保 hreflang 正确，每种语言独立可索引
- 创建 `llms.txt` 和 `sitemap.txt` 帮助 AI 爬虫快速定位核心页面（模板见 `templates/llms-txt-template.md`）
...
90 天闭环优化：监测、复盘、迭代
  ...
  → 验收：品牌实体在 AI 搜索中呈现完整 CITE 六大维度画像
```
...
> 版本：2.1.0 | 融合 Corey Haines SEO Audit（MIT）+ 姚金刚 yao-geo-skills 方法论 + GEOFlow 最佳实践（Apache 2.0）| AI 搜索场景重写
> 维护：玄老（GEO 知识策展）+ 药老（架构决策）
> 新增（v2.1）：AI 爬虫完整 UA 列表、llms.txt 标准、GEOFlow 最佳实践提取
