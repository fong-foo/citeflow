...
ANALYSIS_GUIDE = {
    ...
    3: {"name": "引用源质量差", "framework": "检查高权威平台是否有品牌页面；同时检查AI引用的内容是否准确描述了品牌（paper_020:引用准确性验证）"},
    4: {"name": "引用源单一", "framework": "识别主来源，在其他平台建内容。注意RAG检索本质是概率性的（paper_021）——单一来源意味着AI随机选择时品牌曝光极不稳定"},
    6: {"name": "竞品维度劣势", "framework": "差距>20用差异化，10-20用追赶。注意检查跨查询稳定性——优化一个查询可能伤害另一个（paper_012）"},
    8: {"name": "引用不稳定", "framework": "跨引擎源重叠率低说明RAG检索对品牌内容随机性高（paper_014+021）。根因：内容在不同引擎的索引中分布不均"},
    9: {"name": "多模态盲区", "framework": "视觉密集型行业（时尚/美妆/家居）在VLM搜索引擎中图像质量+图文一致性是关键（paper_011）。检查产品图分辨率/多角度/品牌一致性"},
    10: {"name": "行业影响力弱", "framework": "A类引用率低说明AI不会主动推荐"},
    12: {"name": "引擎差异异常", "framework": "分析最友好/最不友好的引擎。不同引擎有各自的引用偏好模式——需要引擎特定策略而非一刀切（paper_020:引擎特定策略）"},
    13: {"name": "AI认知偏差", "framework": "对比market_perception和brand_profile"},
    14: {"name": "竞品胜负矩阵", "framework": "聚合维度打分，计算差距"},
    15: {"name": "竞品异常占位", "framework": "竞品引用率高但权威源占比低——可能是对抗优化手段（paper_022/026/030）。分析竞品是否在用prompt injection/STS注入/嵌入毒化。建议：强化权威信号+多引擎交叉验证"},

    # 分析框架规则（不独立触发，作为通用指导注入）
    "framework_priority": {"name": "评分优先级", "content": "优先修: 容易修+影响大的维度。内容力=容易，品牌力=中等，技术力=难，产品力=最难"},
    "framework_severity": {"name": "严重程度判定", "content": "critical: 引用率<30% 或 对齐度<60 或 综合分<40。warning: 引用率30-60% 或 来源质量/多样性问题 或 跨引擎不稳定 或 多模态盲区。healthy: 引用率>80% 且 对齐度>80% 且 综合分>70"},
    "framework_benchmark": {"name": "行业基准对比", "content": "对比P25/P50/P75。>P75=领先，P50-P75=中上，P25-P50=中下，<P25=落后。注意：基准是估算值"},
    "framework_cross_query": {"name": "跨查询稳定性", "content": "来自paper_012(IF-GEO)：一次内容优化可能提升目标查询但伤害其他查询。检查被优化页面在非目标查询中的引用变化。优先采用diverge-then-converge策略——先发散识别每个查询的独立偏好，再收敛融合为全局优化方案"},
    "framework_multimodal": {"name": "多模态优化意识", "content": "来自paper_011(Multimodal GEO)：VLM搜索引擎同时编码图像和文本。视觉密集型行业需关注：产品图分辨率/多角度展示/品牌视觉一致性/文本-图像内容一致性。攻击者可利用多模态协同进行排名操纵——监控竞品是否在用此手段"},
    "framework_feature_level": {"name": "特征级优化优先", "content": "来自paper_019(Think Before Writing)：优化时优先考虑页面结构特征、内容属性、语言风格三个维度，而非纠结于单词级别的微调。先定特征配置（需要什么结构元素+内容属性+语言风格），再填充内容"},
    "framework_adversarial": {"name": "对抗攻击意识", "content": "来自paper_022-031(风险操纵系列)：竞品可能通过prompt injection、STS文本注入、嵌入空间毒化等手段操纵AI搜索排名。检测竞品引用率异常+权威源占比低是攻击信号。防御策略：强化权威信号、多引擎交叉验证、监控竞品异常行为"},
    "framework_multi_dim": {"name": "多维度影响力", "content": "来自paper_035(CC-GSEO)：被引用≠被推荐≠被准确描述。三个维度：Exposure(是否被引用)、Faithful Credit(引用是否准确)、Causal Impact(引用是否不可替代)。仅优化引用率而忽略准确性和不可替代性=虚假GEO增长"},
    "framework_structural": {"name": "结构化优先", "content": "来自paper_034(SAGEO Arena)：结构化信息驱动AI检索(+22%命中率)，正文驱动AI生成。两者需互补而非替代。先优化结构(H1-H3/Schema/元数据)，再优化正文(声明-证据结构)，单方面优化正文可能伤害检索(-9%)"},
}
...
def build_rule_section(triggered: list[dict]) -> str:
    ...
    has_instability = any(r["rule_id"] in (4, 8) for r in triggered)
    has_multimodal = any(r["rule_id"] in (9,) for r in triggered)
    has_adversarial = any(r["rule_id"] in (15,) for r in triggered)
    if has_competitor:
    ...
    if has_instability:
        frameworks_to_inject.append(("framework_cross_query", "跨查询稳定性（paper_012）"))
    if has_multimodal:
        frameworks_to_inject.append(("framework_multimodal", "多模态优化意识（paper_011）"))
    # 规则6(竞品维度劣势) 或 规则12(引擎差异) 触发时注入特征级优化框架
    if any(r["rule_id"] in (6, 12) for r in triggered):
        frameworks_to_inject.append(("framework_feature_level", "特征级优化优先（paper_019）"))
    if has_adversarial:
        frameworks_to_inject.append(("framework_adversarial", "对抗攻击意识（paper_022-031）"))
    if has_instability or has_competitor:
        frameworks_to_inject.append(("framework_structural", "结构化优先（paper_034）"))
