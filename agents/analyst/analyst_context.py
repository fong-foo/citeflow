...
# _DEFAULT_BENCHMARKS — industry benchmark data (defaults, override via env var)
import json, os
_DEFAULT_BENCHMARKS = json.loads(r'''{"_default": {"citation_rate": {"p25": 20, "p50": 40, "p75": 60}}, "beauty": {"citation_rate": {"p25": 25, "p50": 50, "p75": 70}}, "fashion": {"citation_rate": {"p25": 25, "p50": 50, "p75": 70}}, "technology": {"citation_rate": {"p25": 15, "p50": 35, "p75": 55}}, "consumer_goods": {"citation_rate": {"p25": 20, "p50": 45, "p75": 65}}, "home": {"citation_rate": {"p25": 20, "p50": 45, "p75": 65}}, "outdoor": {"citation_rate": {"p25": 20, "p50": 45, "p75": 65}}}''')
from agents.tools.brand_profiler import map_industry_category
...
def build_context(probe_output: dict) -> dict:
    ...
    benchmark = _DEFAULT_BENCHMARKS.get(industry_category, _DEFAULT_BENCHMARKS["_default"])
    ...
    return {
        ...
        "engine_results": {
            engine: {
                ...
                "top_sources": _extract_source_domains(result.get("sources", {})),
            }
...
def _extract_source_domains(sources: dict) -> list:
    """从sources dict提取域名列表，供规则8计算跨引擎源重叠率。
    
    sources格式: {"domain1.com": {"mentions": 3, "type": "news"}, ...}
    输出: ["domain1.com", "domain2.com", ...]
    """
