...
## 内容模版（Content Templates）

除了 P0/P1/P2 处方清单外，你还需输出一个独立的 content_templates 对象。
这 8 个字段全部是英文，用户可直接复制到网站上使用。

生成规则：
1. page_title / meta_description：基于品牌定位和诊断发现的 AI 认知盲点，重写为含关键词的英文
2. about_us_opening：从品牌自述中提取核心价值，改写为 2-3 句英文企业介绍
3. social_bio：3-4 行英文社媒简介，含 emoji 和行业关键词
4. faq_pairs：2-4 个 FAQ 问答对，针对诊断发现的内容缺口（如竞品对比、兼容性、认证）
5. keywords_to_emphasize：5-8 个应该强化的英文关键词（来自品牌自述差异化和 AI 盲点）
6. keywords_to_avoid：3-5 个会强化 AI 负面认知的关键词（来自 AI 认知画像中的 weaknesses）
7. key_content_action：一句话概括最优先的内容改动（改什么→改成什么→在哪改）

如果品牌自述数据不足（官网爬取失败），content_templates 所有字段填 null。
...
## 禁止事项
- 禁止直接复制 Few-Shot 示例中的具体数值（如引用率数字、权重系数等）
- evidence 字段中的数值必须来自知识库注入内容或诊断数据，不得来自 Few-Shot 示例
- 如果知识库未提供具体数值，用定性描述代替（如"显著提升"而非"提升0.2144"）"""
...
正确输出:
{
  "prescription": [
    {
      ...
      "evidence": "CITE·Content 维度·结构化数据子策略。论文6, Section 4.4: 宏观结构对引用率有显著贡献。论文3, Section 4.3: 可提取的数值事实提升引用率",
    ...
    },
    {
      ...
      "evidence": "CITE·Trust 维度·第三方背书子策略。论文17: Earned Media权威信号可乘数级提升信任评级。论文3, Section 4.4: 评测平台有显著引用权重贡献。证据成熟度：当前品牌主张多为D级（无第三方验证的'行业领先'类声称），目标：将D级主张升级为B级（通过G2/Trustpilot独立验证）",
  ...
  "content_templates": {
    "page_title": "UGREEN — Intelligent Charging Solutions for Every Device",
    "meta_description": "UGREEN provides intelligent charging solutions trusted by millions of users worldwide. GaN chargers, USB-C hubs, and charging accessories with industry-leading safety certifications.",
    "about_us_opening": "UGREEN provides intelligent charging solutions trusted by millions of users worldwide. From GaN fast chargers to USB-C hubs, every product is engineered for reliability and safety — backed by over 100 international certifications including UL, CE, and FCC. We don't just make accessories. We power your digital life.",
    "social_bio": "🔌 Intelligent charging solutions\n⚡ GaN fast chargers | USB-C hubs | Cables\n✅ UL/CE/FCC certified | Millions of users\n🌍 Available in 100+ countries\n#UGREEN #ChargingSolution #GaN",
    "faq_pairs": [
      {"q": "How does UGREEN compare to Anker?", "a": "UGREEN offers comparable charging speeds and build quality at competitive prices. Our 100W GaN charger matches Anker's charging speed while including an extra USB-C port. Both brands use GaN technology for compact size and efficient heat management."},
      {"q": "Are UGREEN chargers safe for iPhone 16?", "a": "Yes. All UGREEN USB-C chargers are MFi-certified and fully compatible with iPhone 16. They support USB PD 3.0 fast charging and include over-current, over-voltage, and temperature protection."},
      {"q": "What is the warranty on UGREEN products?", "a": "UGREEN offers a 24-month warranty on all charging products, backed by lifetime technical support. For any issues, contact our support team via email or live chat."}
    ],
    "keywords_to_emphasize": ["intelligent charging", "GaN fast charger", "USB-C hub", "MFi certified", "UL certified", "millions of users", "100+ countries", "24-month warranty"],
    "keywords_to_avoid": ["cheap charger", "generic cable", "knockoff"],
    "key_content_action": "把 About Us 首段从'UGREEN makes charging accessories'改为'UGREEN provides intelligent charging solutions trusted by millions of users worldwide'，在产品页每个产品卡片下方添加 FAQ 折叠面板"
  }
}"""
