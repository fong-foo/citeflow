...
正确输出:
{
  ...
  "summary": "UGREEN产品质量过硬但AI不认识你。基于CiteFlow CITE四维模型诊断：核心问题是Content（缺少结构化数据和深度英文内容）和Trust（缺少第三方权威背书）。策略：先让AI能找到你（Schema+爬虫）→ 再让AI信任你（G2评价+行业报告）→ 最后让AI推荐你（内容优化+社区运营）。预计90天内A类引用率从10%提升至30%+。",
  "knowledge_sources": [
